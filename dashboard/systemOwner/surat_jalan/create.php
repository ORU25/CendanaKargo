...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_surat_jalan'])) {
    ...
    // Validasi cabang
    $stmt_cabang = $conn->prepare("SELECT id, kode_cabang, nama_cabang FROM Kantor_cabang WHERE kode_cabang IN (?, ?)");
    $stmt_cabang->bind_param('ss', $cabang_asal, $cabang_tujuan);
    $stmt_cabang->execute();
    $result_cabang = $stmt_cabang->get_result();
    
    if ($result_cabang->num_rows < 2) {
    ...
    while ($row = $result_cabang->fetch_assoc()) {
        $cabangs[$row['kode_cabang']] = $row;
    }
    $stmt_cabang->close();
    
    $cabang_asal_data = $cabangs[$cabang_asal] ?? null;
    $cabang_tujuan_data = $cabangs[$cabang_tujuan] ?? null;
    ...
    $prefix = strtoupper($cabang_asal) . strtoupper($cabang_tujuan);
...
$stmt_sj = $conn->prepare("SELECT * FROM Surat_jalan WHERE id = ? AND status = 'draft'");
