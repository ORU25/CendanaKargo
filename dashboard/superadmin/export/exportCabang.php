<?php
// File: dashboard/superadmin/export/exportCabang.php
session_start();
include '../../../config/database.php';

// Ambil parameter cabang dari session (untuk superAdmin)
...
if (empty($cabang)) {
    header("Location: ../index.php?error=no_cabang");
    exit();
}
...
// Cash dari cabang + bayar_ditempat POD ke cabang (berdasarkan tanggal pengambilan)
// Buat date condition untuk pengambilan
$date_condition_pengambilan = str_replace('p.tanggal', 'pg.tanggal', $date_condition);
...
$queryCash = "SELECT 
          ...
          SELECT 
            ...
            'bayar_ditempat_pod' AS tipe_cash
          FROM pengiriman p2
...
$stmtCash->bind_param('ss', $cabang, $cabang);
$stmtCash->execute();
...
$stmtTransfer->bind_param('ss', $cabang, $cabang);
...
$stmtbayar_ditempat->bind_param('s', $cabang);
...
$stmtInvoice->bind_param('s', $cabang);
$stmtInvoice->execute();
...
echo "<tr style='background:#dc3545; color:white; font-weight:bold;'>
        <th colspan='15'>LAPORAN PENDAPATAN CASH - CABANG " . strtoupper($cabang) . "</th>
      </tr>";
echo "<tr><td colspan='15' style='background:#f8d7da;'>Periode: " . htmlspecialchars($periode_display) . "</td></tr>";
echo "<tr><td colspan='15' style='background:#fff3cd; font-style:italic;'>Cash dari cabang ini + BT POD ke cabang ini</td></tr>";
...
echo "<tr style='background:#0d6efd; color:white; font-weight:bold;'>
        <th colspan='15'>LAPORAN PENDAPATAN TRANSFER + INVOICE LUNAS - CABANG " . strtoupper($cabang) . "</th>
      </tr>";
echo "<tr><td colspan='15' style='background:#cfe2ff;'>Periode: " . htmlspecialchars($periode_display) . "</td></tr>";
echo "<tr><td colspan='15' style='background:#fff3cd; font-style:italic;'>Transfer dari cabang ini + Invoice yang dilunaskan (berdasarkan tanggal pelunasan)</td></tr>";
...
echo "<tr style='background:#ffc107; color:black; font-weight:bold;'>
        <th colspan='15'>LAPORAN PENDAPATAN BAYAR DITEMPAT - CABANG " . strtoupper($cabang) . "</th>
      </tr>";
echo "<tr><td colspan='15' style='background:#fff3cd;'>Periode: " . htmlspecialchars($periode_display) . "</td></tr>";
echo "<tr><td colspan='15' style='background:#fff3cd; font-style:italic;'>Bayar Ditempat dari cabang ini (semua status)</td></tr>";
...
echo "<tr style='background:#dc3545; color:white; font-weight:bold;'>
        <th colspan='15'>LAPORAN INVOICE BELUM DIBAYAR - CABANG " . strtoupper($cabang) . "</th>
      </tr>";
echo "<tr><td colspan='15' style='background:#f8d7da;'>Periode: " . htmlspecialchars($periode_display) . "</td></tr>";
echo "<tr><td colspan='15' style='background:#fff3cd; font-style:italic;'>Invoice dari cabang ini yang belum dilunaskan (hutang)</td></tr>";
